...
describe('the watched character’s mirror', () => {
  ...
  it('walks off mid-swing instead of finishing the clip first', () => {
    watch(false)
    remotePlayerManager.removePlayer(ID)
    remotePlayerManager.initPlayer(ID, { x: 0, y: 0, z: 0 }, 0)
    ...
    remotePlayerManager.setTargetPosition(ID, { x: 10, y: 0, z: 0 }, 0, true)
    remotePlayerManager.update(200)
    expect(remotePlayerManager.players.get(ID)?.position.x).toBeGreaterThan(0)
  })
})
