<script lang="ts">
  ...
  import { spectatorHasEnteredWorld } from '../stores/spectatorReadiness'
  ...
  // A spectator has no AccountCharacter, but the mirror still feeds the level,
  // HP and hunger this row draws — so it is the only vitals readout the
  // observer window gets, and the app shell no longer duplicates it.
  const showStatusRow = $derived(
    !$mapEditorMode &&
      (selectedCharacter !== null ||
        (isObserver && spectatorHasEnteredWorld(currentPlayerHp)))
  )
    ...
    {#if showStatusRow}
      <div class="status-row">
        <LevelBadge
          ...
          hp={playerHp}
          maxHp={playerMaxHp}
          str={selectedCharacter?.attributes.str ?? null}
        />
        <HungerIndicator />
      </div>
    {/if}
