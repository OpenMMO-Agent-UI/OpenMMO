...
#[test]
fn a_worker_bites_on_the_move_once_per_hunger_reading() {
    let mut s = state_at(0.0, 0.0);
    bag(&mut s, "apple", 2);
    s.self_hunger = Some((200, HungerState::Hungry));
    assert_eq!(snack(&s), None, "only while a worker drives");
    s.eats_on_the_move = true;
    assert_eq!(snack(&s), Some(1));
    s.snacked_at = Some(200);
    assert_eq!(snack(&s), None, "the last bite is still unanswered");
    s.self_hunger = Some((900, HungerState::Normal));
    assert_eq!(snack(&s), None, "can sprint again");
}
...
fn eating_waits_for_the_lost_sprint_and_needs_food_in_the_bag() {
    ...
    bag(&mut s, "apple", 1);
    s.self_hunger = Some((900, HungerState::Normal));
    assert_eq!(should_eat(&s, &cfg(), false), None);
    ...
    assert_eq!(should_eat(&s, &cfg(), false).as_deref(), Some("apple"));
    s.self_hunger = Some((200, HungerState::Hungry));
    assert_eq!(should_eat(&s, &cfg(), false).as_deref(), Some("apple"));
    s.self_bag.clear();
    s.self_hunger = Some((10, HungerState::Weak));
    assert_eq!(should_eat(&s, &cfg(), false), None, "nothing to eat");
    // A fisher standing in its own supply should not starve on the way home.
    bag(&mut s, "raw_minnow", 3);
    assert_eq!(should_eat(&s, &cfg(), false).as_deref(), Some("raw_minnow"));
}

/// One bite lifts satiation by one food's nutrition, so a meal that stopped
/// at the first bite past hungry was back at hungry a minute later. Once
/// started, a meal runs up to the target — and waits for the server to
/// confirm each bite before taking the next.
#[test]
fn a_meal_runs_up_to_the_target_one_confirmed_bite_at_a_time() {
    let cfg = WorkerConfig {
        satiation_target: 500,
        ..cfg()
    };
    let mut s = state_at(0.0, 0.0);
    bag(&mut s, "raw_minnow", 10);
    let mut meal = Meal::default();

    s.self_hunger = Some((320, HungerState::Normal));
    assert_eq!(meal.bite(&s, &cfg), None, "fed enough: no meal starts");
    assert!(!meal.eating);

    s.self_hunger = Some((300, HungerState::Normal));
    assert_eq!(meal.bite(&s, &cfg).as_deref(), Some("raw_minnow"));
    assert!(meal.eating);
    assert_eq!(meal.bite(&s, &cfg), None, "the bite is not confirmed yet");

    s.self_hunger = Some((340, HungerState::Normal));
    assert_eq!(
        meal.bite(&s, &cfg).as_deref(),
        Some("raw_minnow"),
        "past hungry, but the meal is not done"
    );
    s.self_hunger = Some((499, HungerState::Normal));
    assert_eq!(meal.bite(&s, &cfg).as_deref(), Some("raw_minnow"));
    s.self_hunger = Some((500, HungerState::Normal));
    assert_eq!(meal.bite(&s, &cfg), None, "target reached");
    assert!(!meal.eating);
    s.self_hunger = Some((450, HungerState::Normal));
    assert_eq!(
        meal.bite(&s, &cfg),
        None,
        "no new meal until the sprint goes"
    );
}

#[test]
fn the_satiation_target_is_held_between_the_first_bite_and_full() {
    use onlinerpg_shared::hunger::{NORMAL_MIN, SATIATION_MAX};
    let at = |v| WorkerConfig {
        satiation_target: v,
        ..cfg()
    };
    assert_eq!(satiation_target(&at(0)), NORMAL_MIN + 1);
    assert_eq!(satiation_target(&at(700)), 700);
    assert_eq!(satiation_target(&at(5000)), SATIATION_MAX);
    assert_eq!(
        satiation_target(&WorkerConfig::default()),
        onlinerpg_shared::hunger::SATIATION_START
    );
}

#[test]
fn the_cheapest_meal_is_eaten_first() {
    let mut s = state_at(0.0, 0.0);
    s.self_hunger = Some((10, HungerState::Weak));
    bag(&mut s, "trophy_golden_sturgeon", 1);
    bag(&mut s, "river_salmon", 1);
    bag(&mut s, "raw_minnow", 1);
    assert_eq!(should_eat(&s, &cfg(), false).as_deref(), Some("raw_minnow"));
    s.self_bag.retain(|i| i.item_def_id != "raw_minnow");
    assert_eq!(
        should_eat(&s, &cfg(), false).as_deref(),
        Some("river_salmon")
    );
}
...
fn whatever_hits_us_becomes_the_target_however_big_it_is() {
    ...
    assert_eq!(
        attackers_in(&[hit(me)], Some(&me)),
        vec!["orc-1".to_string()]
    );
    assert_eq!(
        attackers_in(&[hit(PlayerId::from(2))], Some(&me)),
        Vec::<String>::new()
    );
    assert_eq!(attackers_in(&[], Some(&me)), Vec::<String>::new());
}

/// Two monsters on us: the first is fought to the end, and the second is
/// taken up the moment it drops — nearest first, dead and vanished ones
/// forgotten.
#[test]
fn the_pack_is_answered_one_at_a_time_nearest_next() {
    let me = PlayerId::from(1);
    let hit = |id: &str| ServerMessage::MonsterAttackedPlayer {
    ...
    // A swing that missed is still a monster on us.
    let miss = |id: &str| ServerMessage::MonsterAttackedPlayer {
        monster_id: id.to_string(),
        player_id: me,
        hit: false,
        roll: 2,
    ...
    let mut s = state_at(0.0, 0.0);
    see(&mut s, monster("far", "orc", 6.0, 0.0));
    see(&mut s, monster("near", "goblin", 1.5, 0.0));
    let mut pack = Pack::default();
    pack.hit_by(&[miss("far"), miss("near"), hit("far")], Some(&me));
    pack.prune(&s);
    assert_eq!(pack.next(&s).as_deref(), Some("near"));

    pack.drop("near");
    assert_eq!(pack.next(&s).as_deref(), Some("far"));

    // A hit from something no longer in sight is not a target.
    pack.hit_by(&[hit("ghost")], Some(&me));
    pack.prune(&s);
    assert_eq!(pack.next(&s).as_deref(), Some("far"));
    s.nearby_monsters.get_mut("far").unwrap().health = 0;
    pack.prune(&s);
    assert_eq!(pack.next(&s), None);
}
...
#[tokio::test]
async fn a_kill_cuts_the_swing_cooldown_short() {
    let mut s = state_at(0.0, 0.0);
    s.nearby_monsters
        .insert("m".into(), monster("m", "goblin", 1.0, 0.0));
    let state = Arc::new(Mutex::new(s));
    let started = Instant::now();
    assert!(!wait_tick(&state, Some("m"), Duration::from_millis(120)).await);
    ...
    state
        ...
        .health = 0;
    let started = Instant::now();
    assert!(wait_tick(&state, Some("m"), Duration::from_secs(5)).await);
    assert!(started.elapsed() < Duration::from_secs(1));
}

#[test]
fn sellable_loot_in_reach_is_picked_up_without_a_kill() {
    let mut s = state_at(0.0, 0.0);
    let labels = labels::BagLabels {
        dropable: vec!["leather_helmet".into()],
        ..Default::default()
    };
    s.remember_ground_item(ground_item(1, "old_boot", 1.0, 0.0, 0));
    s.remember_ground_item(ground_item(2, "leather_helmet", 2.0, 0.0, 0));
    s.remember_ground_item(onlinerpg_shared::inventory::GroundItem {
        dropped_by: Some(onlinerpg_shared::PlayerId::from(2)),
        ..ground_item(3, "gold_ring", 3.0, 0.0, 0)
    });
    s.remember_ground_item(ground_item(4, "gold_ring", 4.0, 0.0, 0));
    s.remember_ground_item(ground_item(5, "gold_ring", SWEEP_RADIUS + 1.0, 0.0, 0));
    let pick = |s: &SharedState| sellable_on_ground(s, &labels).map(|i| i.instance_id);
    assert_eq!(pick(&s), Some(4));
    s.loot_given_up.insert(4);
    assert_eq!(pick(&s), None);
}
...
fn the_fisher_casts_at_the_nearest_water_and_stops_short_of_it() {
    ...
    assert_eq!((sx, sz), (0.0, 2.0), "walk to the shore, not into the sea");
    assert_eq!(fisher::nearest_water(0.0, 0.0, &[(1.0, 1.0, false)]), None);
}

fn fisher_cfg() -> WorkerConfig {
    WorkerConfig {
        kind: WorkerKind::Fisher,
        ..WorkerConfig::default()
    }
}
...
#[test]
fn a_fisher_without_a_rod_goes_to_town_and_buys_one() {
    let mut s = state_at(0.0, 0.0);
    s.self_hunger = Some((900, HungerState::Normal));
    assert!(!fisher::has_rod(&s));
    assert!(should_town_trip(&s, &fisher_cfg()));
    assert!(
    ...
    assert!(town_business(&s, &fisher_cfg(), &labels)
        .iter()
        .any(|st| *st == Step::Buy(fisher::ROD.to_string())));
    assert!(why_nothing(&s, &fisher_cfg(), &labels).contains("no fishing rod"));

    // One rod is the whole stock, wherever it is carried.
    bag(&mut s, fisher::ROD, 1);
    assert!(fisher::has_rod(&s));
    assert!(!should_town_trip(&s, &fisher_cfg()));
    ...
    s.self_equipped.insert(
        ...
        ItemInstance {
            instance_id: 9,
            item_def_id: fisher::ROD.to_string(),
            quantity: 1,
            enchant: 0,
            cape_color: None,
            cape_texture: None,
...
fn a_rod_the_purse_cannot_cover_is_not_ordered() {
    ...
    assert_eq!(rod_to_buy(&s, &fisher_cfg()), None);
    assert_eq!(
        town_business(&s, &fisher_cfg(), &labels::BagLabels::default()),
        Vec::new()
    );
}

#[test]
fn the_fisher_walks_to_its_spot_before_looking_for_water() {
    assert_eq!(fisher::spot_leg(None, 0.0, 0.0), None);
    let spot = Some((100.0, 0.0));
    assert_eq!(
        fisher::spot_leg(spot, 0.0, 0.0),
        Some(Step::Walk { x: 100.0, z: 0.0 })
    );
    assert_eq!(fisher::spot_leg(spot, 95.0, 0.0), None, "near enough");
    assert_eq!(fisher::spot(&fisher_cfg()), None);
    assert_eq!(
        fisher::spot(&WorkerConfig {
            fishing_x: Some(1.0),
            fishing_z: Some(2.0),
            ..fisher_cfg()
        }),
...
#[test]
fn a_remembered_cast_is_repeated_walked_back_to_or_forgotten() {
    let samples = [(0.0, 12.0, true), (0.0, 6.0, true)];
    let cast = fisher::plan_cast((0.0, 0.0), &samples).expect("water found");
    assert_eq!(cast.water, (0.0, 6.0));
    assert_eq!(cast.stand, (0.0, 2.0), "the shore, short of the water");
    assert_eq!(fisher::plan_cast((0.0, 0.0), &[(1.0, 1.0, false)]), None);

    let mut mem = fisher::Fishing::default();
    assert_eq!(
        fisher::resume(&mut mem, 0.0, 0.0),
        None,
        "nothing remembered"
    );
    mem = fisher::Fishing {
    ...
    // On the shore: cast again, no rescan.
    assert_eq!(
        fisher::resume(&mut mem, 0.0, 2.0),
        Some(Step::Fish { x: 0.0, z: 6.0 })
    );
    // Sent away (town, a fight): walk back to the shore.
    assert_eq!(
        fisher::resume(&mut mem, 0.0, -30.0),
        Some(Step::Walk { x: 0.0, z: 2.0 })
    );
    // On the shore yet out of cast range: the cast is stale, scan again.
    mem.cast = Some(fisher::Cast {
        stand: (0.0, 0.0),
        water: (0.0, 20.0),
    });
    assert_eq!(fisher::resume(&mut mem, 0.0, 0.0), None);
    assert_eq!(mem.cast, None);
}

#[tokio::test]
async fn the_fisher_drops_marked_junk_where_it_stands() {
    let tick = |kind: WorkerKind| async move {
        let mut s = state_at(0.0, 0.0);
        s.self_hunger = Some((900, HungerState::Normal));
        bag(&mut s, fisher::ROD, 1);
        bag(&mut s, "old_boot", 1);
        bag(&mut s, "clump_of_kelp", 1);
        ...
        next_step(
            ...
            &mut fisher::Fishing::default(),
        )
        .await
    };
    assert_eq!(
        tick(WorkerKind::Fisher).await,
        vec![Step::Drop("old_boot".to_string())],
        "only what is marked, and without waiting for town"
    );
    assert_ne!(
        tick(WorkerKind::Fighter).await,
        vec![Step::Drop("old_boot".to_string())],
        "the fighter drops on its town trip"
    );
}

#[tokio::test]
async fn a_sunken_purse_is_opened_before_anything_else() {
    let mut s = state_at(0.0, 0.0);
    s.self_hunger = Some((900, HungerState::Normal));
    bag(&mut s, fisher::ROD, 1);
    bag(&mut s, "sunken_coin_pouch", 1);
    ...
    .await;
    assert_eq!(steps, vec![Step::Use("sunken_coin_pouch".to_string())]);
}

// --- Config ---

#[test]
fn the_fishing_spot_comes_from_the_worker_table() {
...
"#,
    ...
    assert_eq!(worker.kind, WorkerKind::Fisher);
    assert_eq!(fisher::spot(worker), Some((12.5, -3.0)));
    assert_eq!(fisher::spot(&WorkerConfig::default()), None);
}
...
/// The reins are the dearest thing in the bag. A Sellable mark on them is a
/// slip of the mouse, not an order to sell them.
#[test]
...
fn an_empty_larder_sends_the_worker_home_when_the_sprint_goes() {
    ...
    bag(&mut s, "bread", 1);
    assert!(should_eat(&s, &cfg(), false).is_some());
    assert!(
        !should_town_trip(&s, &cfg()),
...
mod dungeon {
    ...
    use crate::driver::worker::dungeoneer::{
        current_depth, cycle_done, key_owed, step, Run, CHEST_LOOT_TRIES, LEG_PROGRESS, LEG_TRIES,
    };
    ...
    /// The stall detector counts a leg asked for over and over that never
    /// lands. A long approach now asks for the same destination once per
    /// stretch while plainly getting there, and counting those called a
    /// working journey a stall.
    #[test]
    fn a_leg_that_is_getting_somewhere_is_not_a_stall() {
        let (mut s, d) = crypt();
        on_the_surface(&mut s);
        let cfg = cfg(&d);
        let mut run = Run::default();

        for stretch in 0..LEG_TRIES * 2 {
            let steps = step(&mut s, &cfg, &mut run, &marks());
            assert!(
                !matches!(steps.as_slice(), [Step::Idle]),
                "halted after {stretch} stretches of real progress"
            );
            // Each stretch actually moves us on.
            s.self_player.as_mut().unwrap().position.x += LEG_PROGRESS * 3.0;
        }
    }
