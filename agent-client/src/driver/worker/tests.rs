...
#[test]
fn template_worker_config_accepts_a_document_path() {
    let cfg: WorkerConfig =
        toml::from_str("kind = \"template\"\ntemplate_file = \"workers/slime.json\"\n")
            .expect("template config parses");

    assert_eq!(cfg.kind, WorkerKind::Template);
    assert_eq!(cfg.template_file.as_deref(), Some("workers/slime.json"));
}

#[test]
fn template_document_requires_the_supported_format_and_version() {
    let valid = r#"{"format":"openmmo-worker","version":1,"rules":[]}"#;
    assert!(template::Template::parse(valid).is_ok());
    assert!(template::Template::parse(r#"{"format":"other","version":1,"rules":[]}"#).is_err());
    assert!(
        template::Template::parse(r#"{"format":"openmmo-worker","version":2,"rules":[]}"#).is_err()
    );
}

#[test]
fn template_loads_a_worker_document_from_disk() {
    ...
    std::fs::write(
        &path,
        r#"{"format":"openmmo-worker","version":1,"rules":[]}"#,
    )
    .unwrap();

    let loaded = template::Template::load(path.to_str().unwrap());
    std::fs::remove_file(path).unwrap();
    assert!(loaded.is_ok());
}

#[test]
fn template_uses_the_highest_priority_matching_rule() {
    let template = template::Template::parse(
        r#"{
          "format": "openmmo-worker", "version": 1,
          "metadata": {"name": "priority"},
          "rules": [
            {"id":"low", "priority":1, "condition":true,
             "actions":[{"action":"say","message":"low"}]},
            {"id":"high", "priority":10,
             "condition":{"eq":[{"ref":"self.level"},1]},
             "actions":[{"action":"say","message":"high"}]}
          ]
        }"#,
    )
    .unwrap();
...
#[test]
fn template_evaluates_supported_conditions_and_resolves_nearest_attack() {
    let template = template::Template::parse(
        r#"{
          "format":"openmmo-worker", "version":1, "rules":[{
            "id":"hunt", "priority":1,
            "condition":{"all":[
              {"lte":[{"ref":"self.health_pct"},50]},
              {"gte":[{"ref":"self.gold"},20]},
              {"any":[{"lt":[1,0]},{"not":{"eq":[true,false]}}]},
              {"exists":{"select":"bag","where":{"eq":[{"ref":"item.id"},"potion"]}}},
              {"exists":{"select":"players","where":{"lt":[{"ref":"player.distance"},5]}}},
              {"exists":{"select":"ground_items","where":{"eq":[{"ref":"ground_item.item"},"coin"]}}},
              {"exists":{"select":"monsters","where":{"all":[
                {"eq":[{"ref":"monster.kind"},"slime"]},
                {"gt":[{"ref":"monster.health_pct"},0]},
                {"ne":[{"ref":"monster.id"},"far"]}
              ]}}}
            ]},
            "actions":[{"action":"attack","target":{"select":"monsters","where":{"eq":[{"ref":"monster.name"},"slime"]}}}]
          }]}
        "#,
    )
    .unwrap();
    ...
    hurt(&mut state, 50);
    state.self_gold = Some(20);
    bag(&mut state, "potion", 2);
    let mut player = test_player(3.0, 0.0);
    player.id = PlayerId::from(2);
    state.nearby_players.insert(player.id, player);
    state.remember_ground_item(ground_item(7, "coin", 2.0, 0.0, 0));
    see(&mut state, monster("far", "slime", 8.0, 0.0));
...
#[test]
fn template_rejects_unknown_actions_before_deciding() {
    let json = r#"{
      "format":"openmmo-worker", "version":1,
      "rules":[{"id":"bad","condition":true,"actions":[{"action":"teleport"}]}]
    }"#;
...
fn template_accepts_concrete_actions_and_caps_a_decision_at_eight() {
    ...
    let template = template::Template::parse(
        &json!({
            "format":"openmmo-worker", "version":1,
            "rules":[{"id":"many", "condition":true, "actions":actions}]
        })
        .to_string(),
    )
    .unwrap();
...
/// selection asks which eligible monster is the best level match in sight.
/// A kobold underfoot with an ogre 18 m out is those two questions with
/// different answers — the chase leg was abandoned the instant it was issued,
/// re-decided unchanged, and abandoned again, 87 times in three minutes with
/// the fighter standing still throughout. Whatever stops a leg has to be what
/// the next decision swings at.
#[test]
fn what_stops_a_leg_is_what_the_next_decision_swings_at() {
    let mut s = state_at(0.0, 0.0);
    s.self_player.as_mut().unwrap().level = 18;
    see(&mut s, monster("kobold-underfoot", "kobold", 10.0, 0.0));
    see(&mut s, monster("ogre-out", "ogre", 18.0, 0.0));

    assert_eq!(
        fighter::eligible_target(&s, &cfg()).as_deref(),
        Some("ogre-out"),
        "the level match is what a chase would walk to"
    );
    assert!(
        prey_in_reach(&s, cfg().level_margin),
        "and the kobold is what would stop that walk"
    );
    assert_eq!(
        fighter::step(&s, &cfg(), false, &mut fighter::Patrol::default()),
        vec![Step::Attack("kobold-underfoot".to_string())],
