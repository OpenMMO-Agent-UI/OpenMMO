//! Monster fighter: hunt the nearest weakest eligible monster around an
//! anchor point, loot the kill, and patrol the ground nearby when nothing is
//! worth attacking.
...
/// The monster worth walking to: the weakest one standing inside the patrol
/// circle, nearest first among equals. What lies outside it is somebody
/// else's problem; chasing it is how a fighter wanders off the ground the
/// player picked.
...
/// The best eligible monster in sight wherever it stands. The patrol circle
/// is a surface idea — underground the walls are the leash, and `is_eligible`
/// already keeps the answer to our own floor.
pub(crate) fn target_unleashed(s: &SharedState, cfg: &WorkerConfig) -> Option<String> {
    best_eligible_within(s, cfg, f32::INFINITY, false).map(|m| m.id.clone())
}
...
/// Distance band width: candidates in the same band count as equally close,
/// so health decides between them instead of a metre of walking.
const SAME_REACH: f32 = 5.0;

/// The best eligible monster within `range` of us, optionally restricted to
/// what stands inside the patrol circle.
///
/// Nearest band first, then least health: a pack that has closed on us is
/// taken apart one at a time, finishing whatever is already bleeding instead
/// of spreading damage over monsters that all keep swinging. Level match no
/// longer sorts — `level_margin` has already ruled out the losing fights.
fn best_eligible_within<'a>(
    ...
    let band = |m: &Monster| (m.position.dist_xz_sq(&me).sqrt() / SAME_REACH) as u32;
    s.nearby_monsters
        .values()
...
pub(crate) fn step(
    ...
    if !town_bound && dist_from(anchor, me.x, me.z) > radius {
        return super::ride_to(s, anchor.0, anchor.1);
    }
    ...
    // the abandoned leg is re-decided unchanged and abandoned again — the
    ...
    super::ride_to(s, me.x + dx * ratio, me.z + dz * ratio)
}
