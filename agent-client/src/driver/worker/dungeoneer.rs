...
pub(super) const LEG_TRIES: u32 = 12;

/// How far a leg has to move us to count as having got somewhere.
pub(super) const LEG_PROGRESS: f32 = 2.0;
...
pub(crate) struct Run {
    ...
    leg: Option<String>,
    legs: u32,
    /// Where we stood when that leg was last asked for, so a leg that is
    /// getting somewhere is told from one that is not.
    leg_from: Option<Position>,
...
impl Run {
    ...
    /// Count a leg, and say whether it has stopped being worth repeating. A
    /// leg that moved us is not a repeat however often its goal is asked for:
    /// a long approach is now walked in stretches, all of them asking for the
    /// same destination, and counting those would call a journey that is
    /// plainly working a stall.
    fn walking(&mut self, key: String, me: Option<Position>) -> bool {
        let moved = matches!(
            (me, self.leg_from),
            (Some(now), Some(before))
                if (now.x - before.x).hypot(now.z - before.z) > LEG_PROGRESS
        );
        if self.leg.as_deref() == Some(key.as_str()) && !moved {
            self.legs += 1;
        } else {
            self.leg = Some(key);
            self.legs = 1;
...
fn leg(s: &SharedState, step: Step, cfg: &WorkerConfig, run: &mut Run) -> Vec<Step> {
    if run.walking(
        format!("{step:?}"),
...
pub(crate) fn step(
    ...
    if s.self_floor_level > 0 {
        ...
        return leg(s, Step::ToGround { x, z }, cfg, run);
    }
