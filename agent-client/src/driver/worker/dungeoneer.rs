...
pub(crate) struct Run {
    ...
    sweep_up: bool,
    /// How far through the current floor's sweep tour we are, and which floor
    /// that tour belongs to.
    stop: usize,
...
fn descend(dungeon: &Dungeon, depth: u8, cfg: &WorkerConfig, run: &mut Run) -> Vec<Step> {
    leg(
        Step::Descend {
...
fn surface(dungeon: &Dungeon, depth: u8, cfg: &WorkerConfig, run: &mut Run) -> Vec<Step> {
    if depth == 0 {
        return vec![Step::Idle];
    }
    descend(dungeon, 0, cfg, run)
}
...
pub(crate) fn step(
    ...
    if run.halted_now() {
        return surface(&dungeon, depth, cfg, run);
    }
...
fn farm(
    ...
    if !section.contains(&depth) {
        ...
        return descend(dungeon, goal, cfg, run);
    }
    ...
    if run.sweep_depth != Some(depth) {
        run.sweep_depth = Some(depth);
        run.stop = 0;
        run.saw_any = false;
        run.saw_eligible = false;
    }
    run.saw_any |= s
        .nearby_monsters
        .values()
    ...
    while let Some(stop) = stops.get(run.stop) {
        let arrived = me.is_some_and(|p| (p.x - stop.x).hypot(p.z - stop.z) <= STOP_ARRIVE_RANGE);
        if !arrived {
            run.say(format!(
    ...
    run.restart_sweep();
    descend(dungeon, next_floor(depth, first, last, run), cfg, run)
}
