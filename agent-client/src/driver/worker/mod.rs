...
pub struct WorkerConfig {
    ...
    /// Which dungeon the dungeoneer works, by registry id or name
    /// (data/dungeons.json). Empty takes the shallowest one there is.
    #[serde(default)]
    pub dungeon_id: Option<String>,
    /// Deaths in one nightly cycle before the dungeoneer stops and says so.
    /// A character that cannot survive the section is not made to grind the
    /// XP penalty down all night.
    #[serde(default = "default_death_limit")]
    pub death_limit: u32,
    /// Once a meal has started, keep eating until satiation reaches this.
    /// One meal lifts satiation by its nutrition alone — a raw fish is 40 of
    /// the 1000 — so stopping at the first bite meant eating again a minute
    /// later, forever.
    #[serde(default = "default_satiation_target")]
    pub satiation_target: u32,
    /// Where the fisher works: it walks here first and looks for water from
    /// here. Unset fishes from wherever it stands.
    #[serde(default)]
    pub fishing_x: Option<f32>,
    #[serde(default)]
    pub fishing_z: Option<f32>,
}
...
fn default_death_limit() -> u32 {
    3
}
fn default_satiation_target() -> u32 {
...
impl Default for WorkerConfig {
    fn default() -> Self {
        Self {
            ...
            satiation_target: default_satiation_target(),
            fishing_x: None,
            fishing_z: None,
        }
    }
...
pub(crate) enum Step {
    ...
    /// Walk to a ground-level spot from indoors upstairs, taking the stairs
    /// down on the way. `floor: 0` is the only thing that leaves a storey —
    /// a plain walk is refused up there, and so is a descent.
    ToGround {
        x: f32,
        z: f32,
    },
    /// Walk to a dungeon floor. Depth 0 is the way back up to the surface.
    /// The executor handles the whole trip: the surface leg to the entrance,
    /// then the stair shafts, opening every door it holds a key for.
    Descend {
        dungeon: String,
        depth: u8,
    },
    /// Climb out of whichever dungeon we are standing in. A depth with no
    /// dungeon named is the executor's own "leave where you are", which is
    /// the one leg a worker that does not know a dungeon's name can walk.
    Surface,
    OpenChest,
    Idle,
}
...
impl Step {
    /// Walks and fight openers settle on our own side, so the next decision
    /// need not wait; uses, pickups and casts wait for the server's answer.
    fn ends_in_place(&self) -> bool {
        matches!(
            self,
            Step::Attack(_)
                | Step::Walk { .. }
    ...
    fn action(&self) -> Option<Value> {
        Some(match self {
            ...
            Step::ToGround { x, z } => {
                json!({"type": "move", "x": x, "z": z, "floor": 0, "sprint": true})
            }
            Step::Descend { dungeon, depth } => {
                json!({"type": "move", "target": dungeon, "depth": depth, "sprint": true})
            }
            Step::Surface => json!({"type": "move", "depth": 0, "sprint": true}),
            // "great" is what picks the treasure chest out of a room that
            // also holds clutter chests; the dungeoneer never opens those.
            Step::OpenChest => json!({"type": "open_chest", "target": "great", "sprint": true}),
...
/// Kit the worker lives on — never sold, never dropped. The reins are the
/// dearest thing in the bag; a Sellable mark on them is a slip, not an order.
...
/// Where a meal stops: the configured target, never below the bite that
/// starts one (or a meal would end the tick it began) and never past full.
pub(crate) fn satiation_target(cfg: &WorkerConfig) -> u32 {
    use onlinerpg_shared::hunger::{NORMAL_MIN, SATIATION_MAX};
    cfg.satiation_target.clamp(NORMAL_MIN + 1, SATIATION_MAX)
}
...
/// fisher standing in its own supply should not walk to town starving. The
/// cheapest meal goes first — every fish feeds the same, and a trophy
/// sturgeon is a purse, not a lunch.
///
/// `eating` says a meal is under way: then the bar is the configured target
/// rather than the first bite's, so one sitting takes the worker back up to
/// fed instead of one fish past hungry.
pub(crate) fn should_eat(s: &SharedState, cfg: &WorkerConfig, eating: bool) -> Option<String> {
    ...
    let bar = if eating {
        satiation_target(cfg)
    } else {
        onlinerpg_shared::hunger::NORMAL_MIN + 1
    };
    if satiation >= bar {
        return None;
    }
    cheapest_food(s).map(|i| i.item_def_id.clone())
}

fn cheapest_food(s: &SharedState) -> Option<&onlinerpg_shared::inventory::ItemInstance> {
    s.self_bag
        .iter()
        .filter(|i| matches!(category(&i.item_def_id), Some("food") | Some("fish")))
        .min_by_key(|i| {
...
/// The bite to take mid-walk: a worker that has lost the sprint to hunger,
/// and not while an earlier bite is still unanswered.
pub(crate) fn snack(s: &SharedState) -> Option<u64> {
    let (satiation, _) = s.self_hunger?;
    if !s.eats_on_the_move
        || onlinerpg_shared::hunger::can_sprint(satiation)
        || s.snacked_at == Some(satiation)
    {
        return None;
    }
    cheapest_food(s).map(|i| i.instance_id)
}
...
/// The fisher's rod, when it has none in hand or in the bag. One is all it
/// ever needs: the rod is a tool, not a stock.
pub(crate) fn rod_to_buy(s: &SharedState, cfg: &WorkerConfig) -> Option<(String, u32)> {
    if cfg.kind != WorkerKind::Fisher || fisher::has_rod(s) {
        return None;
    }
    restock_buy(s, "fishing_rod", Some(fisher::ROD), 1)
}
...
pub(crate) fn should_town_trip(s: &SharedState, cfg: &WorkerConfig) -> bool {
    // Underground there is no merchant to reach, and no way to reach one that
    // does not throw the descent away. The dungeoneer drops its marked junk
    // where it stands and shops on the way back out.
    ...
    // A fisher without its one tool has nothing to do but buy one.
    if cfg.kind == WorkerKind::Fisher && !fisher::has_rod(s) {
        return true;
    }
    ...
    hungry && should_eat(s, cfg, false).is_none()
}
...
pub(crate) fn loot_candidates(s: &SharedState, near: Position, radius: f32) -> Vec<u64> {
    s.ground_items_in_sight()
        ...
        .filter(|(_, item)| !s.loot_given_up.contains(&item.instance_id))
        .map(|(_, item)| item.instance_id)
        .collect()
}

/// The nearest sellable item within [`SWEEP_RADIUS`] that no player dropped
/// and the app has not marked as junk.
pub(crate) fn sellable_on_ground<'a>(
    s: &'a SharedState,
    labels: &labels::BagLabels,
) -> Option<&'a onlinerpg_shared::inventory::GroundItem> {
    s.ground_items_in_sight()
        .into_iter()
        .take_while(|(dist_sq, _)| *dist_sq <= SWEEP_RADIUS * SWEEP_RADIUS)
        .map(|(_, item)| item)
        .find(|item| {
            item.dropped_by.is_none()
                && !s.loot_given_up.contains(&item.instance_id)
                && !labels.dropable.contains(&item.item_def_id)
                && crate::item_defs::get(&item.item_def_id)
                    .is_some_and(|d| d.base_price.unwrap_or(0) > 0)
        })
}
...
/// Every monster that swung at us in these events, first swing first — a
/// miss counts the same as a hit, it is the same monster on us. Self-defence
/// ignores every eligibility rule, the level margin included.
pub(crate) fn attackers_in(
    ...
    for e in events {
        ...
        {
            if Some(player_id) == me && !out.contains(monster_id) {
                out.push(monster_id.clone());
            }
        }
    }
    out
...
/// The monsters that have hit us and still stand. A surrounded worker
/// answers them one at a time — switching mid-fight leaves the whole pack
/// alive and swinging — but the next one is taken up the moment the last
/// one drops, not after a loot sweep and a fresh decision while it keeps
/// hitting us.
...
impl Pack {
    pub(crate) fn hit_by(
        ...
        for id in attackers_in(events, me) {
    ...
    /// Forget what is dead or out of sight.
    pub(crate) fn prune(&mut self, s: &SharedState) {
        self.ids
            .retain(|id| s.nearby_monsters.get(id).is_some_and(|m| m.health > 0));
    }

    pub(crate) fn drop(&mut self, id: &str) {
    ...
    /// The attacker to answer next: the nearest one still standing, since it
    /// is the one already in reach.
    pub(crate) fn next(&self, s: &SharedState) -> Option<String> {
        let me = s.self_player.as_ref()?.position;
        self.ids
            .iter()
            .filter_map(|id| {
                let m = s.nearby_monsters.get(id)?;
                (m.health > 0).then(|| {
                    let d = (m.position.x - me.x).hypot(m.position.z - me.z);
                    (id.clone(), d)
                })
            })
...
pub async fn worker_driver(
    ...
    state.lock().await.eats_on_the_move = true;
    ...
    // Whatever has hit us and still stands, answered one after another.
    let mut pack = Pack::default();
    let mut last_attack_at = Instant::now() - attack_cooldown;
    let mut dead_since: Option<Instant> = None;
    let mut errand = Errand::Work;
    // The meal in progress, so one sitting runs up to the target.
    ...
    // The dungeon run: which nightfall it belongs to, the keys owed, the
    // deaths spent on it.
    let mut run = dungeoneer::Run::default();
    // The shore the fisher last cast from, walked back to after any errand.
    let mut fishing = fisher::Fishing::default();
    ...
    // reported once instead of five times a second.
    ...
    // Whether the last turn walked or opened a fight; the next decision then follows at once.
    let mut acted = false;
    loop {
        ...
        // but only while nothing else is being fought. Switching mid-fight is
        // what a surrounded worker does all the time, and it leaves the whole
        // pack alive and swinging instead of one corpse and one fight left.
        if attack_target.is_none() {
            if let Some(id) = next_attacker {
        ...
        if let Some(rescue) = self_rescue(&state, &cfg, &mut errand, &mut meal, &label).await {
            take_turn(&state, rescue, &watch, &mut last_turn).await;
            // An escape leaves the pack behind; the next tick must not turn
            // round and take it up again.
        ...
        if let Some(target) = attack_target.clone() {
            if target_down || last_attack_at.elapsed() >= attack_cooldown {
                ...
                // Always sprint: the hunger gate is the only governor a worker needs.
                let keep_fighting = !target_down && tick_combat(&state, &target, Some(true)).await;
                attack_target = keep_fighting.then_some(target.clone());
                last_attack_at = Instant::now();
                if attack_target.is_none() {
                    // The rest of the pack comes before the loot: it is still
                    // hitting us, and the drops are not going anywhere. One
                    // site covers a melee's worth of corpses.
                    ...
                    if let Some(id) = next {
                        info!("[{label}] Worker: {target} down, turning on {id}");
                        attack_target = Some(id);
                    }
                }
            }
        ...
        // change made while the worker runs is picked up on this trip. The
        // fisher drops where it stands, so it reads them every tick.
        if errand != Errand::Work || cfg.kind == WorkerKind::Fisher {
            labels = labels::labels_from_prompt(instance_prompt.as_deref());
        }
        run.observe_epoch(state.lock().await.night_epoch);
        let step = next_step(
            &state,
            &cfg,
            &mut errand,
        ...
        .await;
        // What the panel reads to tell a working descent from a stuck one.
        // The action JSON alone cannot say which key is owed or how far the
        // sweep has got.
        if let Some(note) = run.take_note() {
            // The log first: a headless run has no spectator attached, and
            // that is exactly the run whose only account of itself is this.
            info!("[{label}] {note}");
            if let Some(w) = watch.as_ref() {
                w.push("progress", note);
            }
        }
        acted = step.iter().any(Step::ends_in_place);
        prefetch_route(&state, &step, &api_base_url, &label).await;
        if let Some(target) = take_turn(&state, step, &watch, &mut last_turn).await {
...
/// Spacing of the points a long walk loads objects around; well under a
/// region's width, so no region the line crosses is skipped.
const PREFETCH_SPACING_M: f32 = 256.0;

/// Load the objects between us and a step's destination before walking there.
/// The route is only planned around what is loaded, and a walk into a region
/// never fetched ran into fences and props it could not see.
async fn prefetch_route(
    state: &Arc<Mutex<SharedState>>,
    steps: &[Step],
    api_base_url: &str,
    label: &str,
) {
    let (world_cache, from, to) = {
        let s = state.lock().await;
        let Some(me) = s.self_player.as_ref().map(|p| p.position) else {
            return;
        };
        let world_cache = Arc::clone(&s.world_cache);
        let to = steps.iter().find_map(|step| match step {
            Step::Walk { x, z } | Step::ToGround { x, z } => Some((*x, *z)),
            Step::Descend { dungeon, .. } => world_cache
                .read()
                .ok()?
                .dungeon_by_id(dungeon)
                .map(|d| (d.entrance.x, d.entrance.z)),
            _ => None,
        });
        let Some(to) = to else { return };
        (world_cache, (me.x, me.z), to)
    };
    let (dx, dz) = (to.0 - from.0, to.1 - from.1);
    let n = (dx.hypot(dz) / PREFETCH_SPACING_M).ceil() as u32;
    let points: Vec<(f32, f32)> = (0..=n)
        .map(|i| {
            let t = if n == 0 { 1.0 } else { i as f32 / n as f32 };
            (from.0 + dx * t, from.1 + dz * t)
        })
        .collect();
    fetch_furniture_around(&world_cache, &points, api_base_url, label).await;
}

/// Sleep up to `tick` in short slices, returning early with `true` once the
/// target is dead or out of sight so a kill is not followed by a full swing
/// cooldown of standing still.
async fn wait_tick(state: &Arc<Mutex<SharedState>>, target: Option<&str>, tick: Duration) -> bool {
    const SLICE: Duration = Duration::from_millis(50);
    let until = Instant::now() + tick.max(SLICE);
    loop {
        tokio::time::sleep(until.saturating_duration_since(Instant::now()).min(SLICE)).await;
        if let Some(id) = target {
            if target_is_down(&*state.lock().await, id) {
                return true;
            }
        }
...
pub(crate) fn target_is_down(s: &SharedState, id: &str) -> bool {
    s.nearby_monsters
        .get(id)
        .is_none_or(|m| m.health == 0 || m.state == onlinerpg_shared::MonsterState::Dead)
}
...
/// A meal under way: it started when the sprint went and ends when satiation
/// reaches the target. `seen` is the satiation the last bite was taken at —
/// the server's `HungerUpdate` lands a tick or two after the `Use`, and a
/// second bite taken on the stale number is one fish too many.
#[derive(Debug, Default)]
pub(crate) struct Meal {
    pub(crate) eating: bool,
    seen: Option<u32>,
}

impl Meal {
    /// The bite to take this tick, if any; keeps the latch in step with it.
    pub(crate) fn bite(&mut self, s: &SharedState, cfg: &WorkerConfig) -> Option<String> {
        let satiation = s.self_hunger.map(|(v, _)| v);
        let food = should_eat(s, cfg, self.eating);
        if food.is_none() {
            self.eating = false;
            self.seen = None;
            return None;
        }
        if self.eating && self.seen == satiation {
            return None;
        }
        self.eating = true;
        self.seen = satiation;
...
async fn self_rescue(
    ...
    meal.bite(&s, cfg).map(|food| vec![Step::Use(food)])
}
...
async fn next_step(
    ...
    run: &mut dungeoneer::Run,
    fishing: &mut fisher::Fishing,
) -> Vec<Step> {
    let mut s = state.lock().await;
    ...
    // A worker whose whole errand is on the surface, stranded off it. Nothing
    // it would ask for from there is even pathed: `resolve_goal_floor` refuses
    // a goal outside the dungeon we stand in, and one off the storey we stand
    // on, before a route is looked for — so the anchor, the town and the water
    // are all equally unreachable, and the worker fights whatever wanders past
    // while never once moving toward what it is for. The usual way in is a
    // worker switched while the dungeoneer was three floors down, and the
    // other is dying: a respawn wakes the character in a bed upstairs.
    if let Some(out) = back_to_the_surface(&s, cfg) {
        return out;
    }
    let pause = town_blocked_until.filter(|p| Instant::now() < p.until);
    let waiting_out_a_failed_trip = pause.is_some();
    // The dungeoneer adds one trigger of its own: an emptied chest is a haul
    // to sell and a descent's worth of potions to replace, however light the
    // bag reads.
    let town_wanted = should_town_trip(&s, cfg) || run.resupply_due();
    if *errand == Errand::Work && !waiting_out_a_failed_trip && town_wanted {
    ...
    if let Some((site, tries)) = *loot_at {
        ...
        match loot.first() {
            ...
            Some(id) => {
                info!("[{label}] Worker: giving up on the loot here");
                s.loot_given_up.insert(*id);
            }
            None => {}
        }
        *loot_at = None;
    }
    if let Some(item) = sellable_on_ground(&s, labels) {
        *loot_at = Some((item.position, 1));
        return vec![Step::Pickup(item.instance_id)];
    }

    // A sunken purse pays out when opened and is dead weight until then.
    if let Some(pouch) = bag_item_in_category(&s, "coin_catch") {
        return vec![Step::Use(pouch)];
    }
    // The fisher drops its marked junk where it stands: a boot off the line
    // is dead weight the moment it lands, and the shore is a long way from
    // town. The fighter keeps its town-trip drop — it is walking there anyway.
    if cfg.kind == WorkerKind::Fisher {
        if let Some(junk) = junk_list(&s, labels).into_iter().next() {
            return vec![Step::Drop(junk)];
        }
    }
    ...
    match cfg.kind {
        ...
        WorkerKind::Fisher => {
            let job = fisher::water_job(&s, cfg);
            drop(s);
            fisher::step(job, fishing).await
        }
        // Which legs are worth abandoning for prey is the dungeoneer's own
        // question — it depends on the phase, and it sets the flag itself so
        // the answer can never drift from what it does next.
        WorkerKind::Dungeoneer => dungeoneer::step(&mut s, cfg, run, labels),
        WorkerKind::None => vec![Step::Idle],
    }
}

/// The leg back to ground level for a worker that has no business off it, or
/// `None` when it is already there. The dungeoneer is exempt: being below
/// ground is its errand, and it has its own way back up.
fn back_to_the_surface(s: &SharedState, cfg: &WorkerConfig) -> Option<Vec<Step>> {
    if cfg.kind == WorkerKind::Dungeoneer {
        return None;
    }
    match s.self_floor_level {
        0 => None,
        level if level < 0 => Some(vec![Step::Surface]),
        // Indoors upstairs, where a plain walk is refused outright. The spawn
        // point rather than the anchor: it is the one outdoor cell the world
        // guarantees is standable, and from ground level the ordinary rules
        // walk the rest.
        _ => {
            let (x, z) = fighter::spawn_point();
            Some(vec![Step::ToGround { x, z }])
        }
    }
}

/// How long a worker waits between decisions when nothing is being fought.
/// One decision is one lock of the state and a handful of comparisons —
/// cheap enough that the wait is about not flooding the executor, not about
/// CPU — and the walk's own idle tick is the same 200 ms.
const DECISION_TICK: Duration = Duration::from_millis(200);
...
/// How far off a worker walks for sellable loot it did not kill for.
const SWEEP_RADIUS: f32 = 20.0;
...
fn why_nothing(s: &SharedState, cfg: &WorkerConfig, labels: &labels::BagLabels) -> String {
    let load = bag_load_pct(s);
    if cfg.kind == WorkerKind::Fisher && !fisher::has_rod(s) {
        return "no fishing rod, and this merchant has none to sell or the purse cannot cover one"
            .to_string();
    }
...
pub(crate) fn town_business(
    ...
    if let Some((rod, _)) = rod_to_buy(s, cfg) {
        steps.push(Step::Buy(rod));
    }
    if let Some(food) = should_eat(s, cfg, false) {
