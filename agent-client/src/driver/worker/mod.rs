...
/// dearest thing in the bag and the horse is the whole commute; a Sellable
/// mark on them is a slip, not an order.
...
/// A leg long enough to be worth mounting for: a horse rides at
/// `HORSE_MOVE_MULT` (3x), so anything past this saves more than the mount
/// costs. A patrol leg (`PATROL_LEG`, 28 m) sits just under it, and rightly —
/// it is walked to find a fight, and a fight dismounts.
const RIDE_WORTH_IT: f32 = 30.0;

/// Water this deep or deeper is where the server stops letting anyone climb
/// on (`can_ride_here`). Our own `position.y` is the bed height the server
/// samples, and the sea's surface is 0, so the sea's depth is that height
/// negated. A river's surface sits above 0 and this client has no sampler for
/// it, so wading one is the one refusal still left to the server.
const RIDE_MAX_WATER_DEPTH: f32 = 0.6;

/// Whether the server would let us mount where we stand, by its own rules
/// (`can_ride_here`, `server/src/game_state/mounts.rs`): alive, on outdoor
/// ground, in no pose and out of combat. A refused toggle is not free — it
/// spends the turn and answers with a system message — and the conditions it
/// refuses on are exactly the ones a worker sits in for a while at a time:
/// the seconds after a kill, a stretch of a walk through a house. Asking
/// every tick through those would walk the whole leg.
fn can_ride_here(s: &SharedState) -> bool {
    let Some(me) = s.self_player.as_ref() else {
        return false;
    };
    me.health > 0
        // The wire floor, not `self_player.floor_level`: only this one is
        // updated the moment we change floor.
        && s.self_floor_level == 0
        // Sitting, fishing, playing: the server counts any of them as a pose.
        && me.object_type.is_none()
        && !s.in_combat()
        // A horse the server just refused a step on. Climbing back on inside
        // the window puts us straight back in the loop it broke us out of.
        && !s.horse_held()
        && s.storey_at(me.position.x, me.position.z, 0).is_none()
        && me.position.y > -RIDE_MAX_WATER_DEPTH
}

/// Get off, once the server has refused a step we took on horseback. Mounted
/// movement is an arc with no wall slide, so the graze a walker slides past
/// stops the horse dead and drops the whole queue (`tick_player_movement`,
/// `server/src/game_state/player.rs`). The walk gives up after
/// `MAX_CORRECTION_REPATHS`, the worker re-decides the identical leg, the
/// horse takes the same arc into the same corner — and nothing moves until
/// somebody changes something. Climbing off is that something: the leg walks.
///
/// The toggle mirrors itself on send (`send_command`), so this asks once
/// rather than every tick until the echo lands. `horse_held` keeps us off for
/// `REMOUNT_HOLD` afterwards, which is what stops the very next long leg
/// putting us back up.
fn climb_off_a_refused_horse(s: &SharedState) -> Option<Vec<Step>> {
    let me = s.self_player.as_ref()?;
    (me.mounted && s.horse_held())
        .then(|| bag_item_in_category(s, "horse_reins"))?
        .map(|reins| vec![Step::Use(reins)])
}

/// Walk to a point, on horseback when the bag holds reins, the leg is long
/// enough to pay for climbing on, and the ground we stand on will take a
/// mount.
///
/// The reins toggle, so the one rule that is ours alone is never to use them
/// while already riding — that would climb straight back off. The rest mirrors
/// the server, which dismounts by itself the moment a fight starts or we go
/// below ground.
pub(crate) fn ride_to(s: &SharedState, x: f32, z: f32) -> Vec<Step> {
    match reins_for_leg(s, x, z) {
        Some(reins) => vec![Step::Use(reins), Step::Walk { x, z }],
        None => vec![Step::Walk { x, z }],
    }
}

fn reins_for_leg(s: &SharedState, x: f32, z: f32) -> Option<String> {
    let me = s.self_player.as_ref()?;
    if me.mounted || !can_ride_here(s) {
        return None;
    }
    let leg = (x - me.position.x).hypot(z - me.position.z);
    (leg >= RIDE_WORTH_IT)
        .then(|| bag_item_in_category(s, "horse_reins"))
        .flatten()
}
...
/// The monster that just hit us, if any — self-defence ignores every
/// eligibility rule, the level margin included, once the current fight ends.
...
async fn next_step(
    ...
    // Before any leg is chosen: a leg decided while wedged on the horse is a
    // leg that will be refused again.
    if let Some(off) = climb_off_a_refused_horse(&s) {
        return off;
    }
