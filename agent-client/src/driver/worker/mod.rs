...
/// Walk to a point, on horseback when the bag holds reins and the leg is long
/// enough to pay for climbing on.
///
/// The reins toggle, so the one rule that matters here is never to use them
/// while already riding — that would climb straight back off. Everything else
/// is the server's: it refuses a mount in combat, indoors, in water or below
/// ground, and dismounts by itself the moment a fight starts, at no cost to us
/// beyond the turn the refusal spent.
...
fn reins_for_leg(s: &SharedState, x: f32, z: f32) -> Option<String> {
    ...
    if me.mounted || me.floor_level != 0 {
        return None;
    }
