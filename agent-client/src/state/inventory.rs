...
impl SharedState {
    /// A bag item's category, by instance. `send_command` reads it to tell the
    /// reins from every other consumable it forwards.
    pub(super) fn bag_item_category(&self, instance_id: u64) -> Option<&'static str> {
        let item = self
            .self_bag
            .iter()
