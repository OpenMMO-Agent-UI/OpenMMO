...
`ride_to` mounts before any leg of `RIDE_WORTH_IT` (30 m) or more: the commute
back to the anchor, the walk into town and out of a useless one, the run at a
distant target. A patrol leg (`PATROL_LEG`, 28 m) sits just under that line on
purpose — it is walked to find a fight, and the fight would dismount. The reins
are `is_keeper` kit: the dearest thing in the bag is not sold off a stray
Sellable mark.

`can_ride_here` checks the server's own mount rules before spending the toggle
on a refusal — alive, on the surface, in no pose (sitting, fishing, playing),
out of combat, not indoors, not in water — plus never using them while already
up, which would climb straight back off. The refusal is not free: it costs the
turn, and the states it refuses on are ones a worker sits in for a while at a
time, so asking each tick would walk the whole leg. Two of the six the wire
does not report. Combat is kept client-side: `Player::last_combat_at` is
`#[serde(skip)]`, so `SharedState::note_combat` stamps the same clock from the
two messages that carry a blow (`PlayerAttacked` at us, `MonsterAttackedPlayer`
on us) and `in_combat()` reads it against the server's ten-second
`OUT_OF_COMBAT` window. Water is read off our own `position.y`, which is the
bed height the server samples: the sea's surface is 0, so anything below
`-RIDE_MAX_WATER_DEPTH` is too deep to mount. A river's surface sits above 0
and this client has no sampler for it, so wading one is the single case still
left to the server's refusal.
...
A refused step taken on horseback ends the ride. Mounted movement is an arc
with no wall slide (`tick_player_movement`, `server/src/game_state/player.rs`):
on foot a step that grazes an edge is `Slid` along it and the leg carries on,
on horseback the same graze is `Blocked` outright, the whole waypoint queue is
dropped and the server snaps us back. Nothing about retrying changes that — the
route was pathed for a body that walks straight lines, and the horse swings
wide of it by `TURN_RADIUS` the same way every time — so the walk burned its
`MAX_CORRECTION_REPATHS`, the worker re-decided the identical leg, and the
character stood there struggling. `PositionCorrected` arriving while mounted
now arms `hold_the_horse`; `climb_off_a_refused_horse` gets the rider down
before any leg is chosen, and `can_ride_here` keeps them down for
`REMOUNT_HOLD` (30 s) so the very next long leg does not put them straight
back up. The toggle mirrors itself in `send_command` — a dismount is never
refused, and waiting for the echo left the next tick still reading `mounted`
and climbing back on.

The dungeoneer rides the same way, but only on the one leg that can carry a
horse: the surface walk to the entrance that opens every descent. Standing on
the dungeon's own footprint means the executor skips that leg, so there is
nothing to ride; the stair legs start below ground, where the server dismounts
anyone anyway.
...
one, the mount conditions and the out-of-combat clock behind them, the ride to
a dungeon's doorstep and the walked stair legs, a refused step on horseback
ending the ride, the sweep resuming where a fight left the character, a surface
worker left underground or upstairs climbing out first, surplus supply, and
that every emitted step parses as an action). App side:
